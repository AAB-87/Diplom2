...
public class ClaimsFunc {
    ...
    public static void checkCreatedClaimInClaimsBlock(String title) {
        ...
        DataHelper.isDisplayedWithSwipe(onView(withText(title)), 2, true);
    }
    ...
    public static void checkThatFirstFiveClaimsHaveOpenStatus() throws InterruptedException {
        ...
        for (int claimPosition = 0; claimPosition < 3; claimPosition++) {
            onView(DataHelper.withIndex(withId(R.id.claim_list_card), claimPosition)).perform(click());
            Thread.sleep(2000);
        ...
        // проверка 4й заявки
        onView(DataHelper.withIndex(withId(R.id.claim_list_card), 3)).perform(click());
        Thread.sleep(2000);
        ClaimScreen.openStatus.check(matches(isDisplayed()));
        Espresso.pressBack();
        Thread.sleep(2000);
        ...
        onView(allOf(withId(R.id.claim_list_recycler_view), isDisplayed())).perform(actionOnItemAtPosition(2, swipeUp()));
        // проверка 5й заявки
        onView(DataHelper.withIndex(withId(R.id.claim_list_card), 4)).perform(click());
        Thread.sleep(2000);
        ClaimScreen.openStatus.check(matches(isDisplayed()));
        Espresso.pressBack();
        Thread.sleep(2000);
